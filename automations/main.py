...
class Application:
    def __init__(self):
        ...
        self.special_keys = {"Key.shift": p_k.shift, "Key.tab": p_k.tab, "Key.caps_lock": p_k.caps_lock,
                             ...
                             "Key.ctrl_r": p_k.ctrl_r,
                             "Key.shift_r": p_k.shift_r, "Key.enter": p_k.enter, "Key.backspace": p_k.backspace,
                             "Key.right": p_k.right, "Key.down": p_k.down, "Key.left": p_k.left, "Key.up": p_k.up,
                             "Key.page_up": p_k.page_up, "Key.page_down": p_k.page_down, "Key.home": p_k.home,
                             "Key.end": p_k.end, "Key.delete": p_k.delete, "Key.space": p_k.space,
                             'Key.alt_gr': p_k.alt_gr,
    ...
    def play(self):
        ...
        for action in data:
            ...
            if event == "window_change":
                window_title, window_pos = args
                window_to_activate = gw.getWindowsWithTitle(window_title)[0]
                window_to_activate.activate()
                pyautogui.moveTo(window_pos)
    ...
    def on_move(self, x, y):
        if self.window_tracker.track_window():
            active_window = self.window_tracker.last_active_window
            self.data.append({"event": "window_change", "args": (active_window.title, active_window.topleft),
                              "time": time.time() - self.start_time})
        # if prev event is a movement & it's less than .02 seconds old, do nothing
